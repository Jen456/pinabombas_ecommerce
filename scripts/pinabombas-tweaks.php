...
/* ============================================================
 *  CONFIGURACIÓN  ->  CAMBIA ESTE NÚMERO por el real de WhatsApp
 *  Formato internacional SIN "+" ni espacios (Ecuador = 593...)
 * ============================================================ */
if (!defined('PB_WHATSAPP')) define('PB_WHATSAPP', '593999999999');

/* ---------- Precio: "a consultar" si no hay precio ---------- */
...
/* ---------- Helper: link de WhatsApp con mensaje dinámico ---------- */
...
/* ---------- Botón WhatsApp en la grilla (reemplaza el botón) ---------- */
...
/* ---------- Botón WhatsApp en la ficha de producto ---------- */
...
add_action('wp_footer', function () {
    $url = pb_wa_link(null);
    echo '<a href="' . esc_url($url) . '" target="_blank" rel="nofollow" class="pb-wa-float" aria-label="Escríbenos por WhatsApp" title="Escríbenos por WhatsApp">'
...
add_action('wp_head', function () {
    ...
    .pb-wa-float{position:fixed;right:20px;bottom:20px;z-index:99999;width:58px;height:58px;border-radius:50%;background:#25D366;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 18px rgba(0,0,0,.28);transition:transform .15s}
    .pb-wa-float:hover{transform:scale(1.08)}
    .pb-wa-float svg{width:34px;height:34px;fill:#fff}
    table.pb-specs th{width:40%;background:#f5f6f8}
    </style>';
});

/* ---------- Ocultar Admin Bar a usuarios no administradores ---------- */
add_filter('show_admin_bar', function ($show) {
    return current_user_can('manage_options') ? $show : false;
});

/* ---------- Pestaña "Especificaciones técnicas" en la ficha ---------- */
...
/* ---------- Schema JSON-LD Product (sin precio fijo, no muestra $0) ---------- */
add_action('wp_footer', function () {
